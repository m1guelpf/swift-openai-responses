...
@Codable @UnTagged public enum Input: Sendable {
	...
	public struct APIMessage: Equatable, Codable, Sendable {
		/// The role of the message input.
		public let role: Role
		/// Text, image, or audio input to the model, used to generate a response. Can also contain previous assistant responses.
		public let content: Content
		/// The status of the message. Populated when the message is returned via API.
		public let status: Item.Output.Message.Status?

		public init(role: Role, content: Content, status: Item.Output.Message.Status? = nil) {
			self.role = role
			self.status = status
			self.content = content
		}
	...
	/// Text, image, or audio input to the model, used to generate a response. Can also contain previous assistant responses.
	@Codable @UnTagged public enum Content: Equatable, Sendable {
		/// Text, image, or audio input to the model, used to generate a response. Can also contain previous assistant responses.
		@Codable @CodedAt("type") @CodingKeys(.snake_case) public enum ContentItem: Equatable, Sendable {
...
public extension Input.ListItem {
	...
	static func message(role: Role, content: Input.Content) -> Self {
		.message(Input.Message(role: role, content: content))
	}
	...
	static func inputMessage(role: Role, content: Input.Content, status: Item.Output.Message.Status? = nil) -> Self {
		.item(Item.Input.inputMessage(Input.APIMessage(role: role, content: content, status: status)))
	}
	...
	static func outputMessage(content: [Item.Output.Content], id: String, role: Role = .assistant, status: Item.Output.Message.Status) -> Self {
		.item(Item.Input.outputMessage(Item.Output.Message(content: content, id: id, role: role, status: status)))
	}
